...
/// @title Offers V2
/// @author 0xkowloon <0xkowloon@protonmail.com>
/// @notice This module allows users to make ETH/ERC-20 offers for any ERC-721 token or make ETH/ERC-20 collection offers
contract OffersV2 is ReentrancyGuard, UniversalExchangeEventV1, IncomingTransferSupportV1, FeePayoutSupportV1, ModuleNamingSupportV1 {
    ...
    /// @notice Creates a collection offer for an NFT
    /// @param _tokenContract The address of the desired ERC-721 token
    /// @param _currency The address of the ERC-20 token offering, or address(0) for ETH
    /// @param _amount The amount offering
    /// @param _findersFeeBps The bps of the amount (post-royalties) to send to a referrer of the sale
    ...
    function setOfferAmount(
        ...
        _setOfferAmount(_tokenContract, _tokenId, _offerId, _currency, _amount);
    }

    /// @notice Updates the given collection offer for an NFT
    /// @param _tokenContract The address of the offer ERC-721 token
    /// @param _offerId The ID of the offer
    /// @param _currency The address of the ERC-20 token offering, or address(0) for ETH
    /// @param _amount The new amount offering
    function setCollectionOfferAmount(
        ...
        _setOfferAmount(_tokenContract, COLLECTION_OFFER_ID, _offerId, _currency, _amount);
    }
    ...
    /// @param _tokenId The ID of the desired ERC-721 token. If _tokenId is type(uint256).max, it is a collection offer.
    /// @param _offerId The ID of the offer
    function cancelOffer(
        ...
        _cancelOffer(_tokenContract, _tokenId, _offerId);
    }

    /// @notice Cancels and refunds the given collection offer for an NFT
    /// @param _tokenContract The ERC-721 token address of the offer
    /// @param _offerId The ID of the offer
    function cancelCollectionOffer(
        ...
        _cancelOffer(_tokenContract, COLLECTION_OFFER_ID, _offerId);
    }
    ...
    /// @notice Fills a collection offer for an owned NFT, in exchange for ETH/ERC-20 tokens
    /// @param _tokenContract The address of the ERC-721 token to transfer
    ...
    function _fillOffer(
        ...
        uint256 tokenIdKey;
        if (_isCollectionOffer) {
            tokenIdKey = COLLECTION_OFFER_ID;
        } else {
            tokenIdKey = _tokenId;
        }
        Offer memory offer = offers[_tokenContract][tokenIdKey][_offerId];
        ...
        delete offers[_tokenContract][tokenIdKey][_offerId];
    }
}
